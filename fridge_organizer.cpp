...
bool FridgeOrganizer::add_meal_portion(string n_name, string n_expiry) {
	...
	else if(!stacks.empty() && lastStack->size() < stack_capacity)
	{
...
FridgeOrganizer::MealPortion FridgeOrganizer::remove_meal_portion() {
	...
	*m = *lastStack->top();
	...
	MealPortion* garbage = lastStack->top();
	lastStack->pop();
	delete garbage;
	// step6 if the current stack is empty
	if(lastStack->size() == 0)
	{
		stack<MealPortion*>* temp = lastStack;
		stacks.pop_back();
		delete temp;
	}
...
FridgeOrganizer::MealPortion FridgeOrganizer::find_meal_portion_by_expiry() {
	...
	{
		int curIndex = 0;
		for(list<stack<MealPortion*>*>::iterator i = stacks.begin(); i != stacks.end(); i++, curIndex++)
		{
			stack<MealPortion*> curStack = **i; int curStackSize = curStack.size();
			for (int cur_element_index = 0; cur_element_index < curStackSize; ++cur_element_index) {
				if(curStack.top()->expiry.compare(minExpiry) <= 0)
				{
					minExpiry = curStack.top()->expiry;
					min = curStack.top();
				}
				curStack.pop();
			}
...
//RESULTS:
//	
//-= TEST 1: Test FridgeOrganizer constructor and basic item insertion PASSED =-
//
//-= TEST 2: Test stack growth and shrinking with LIFO removal PASSED =-
//
//-= TEST 3: Test find meal portion by earliest expiry date PASSED =-
